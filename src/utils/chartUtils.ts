...
export const prepareBarChartData = (ratings: Rating[]): ChartData & { groupingMode?: string } => {
  ...
  try {
    ...
    if (Object.keys(groupedByDate).length === 0) {
      return {
        labels: [],
        datasets: [
          { label: 'Positive', data: [], backgroundColor: 'rgb(75, 192, 192)' },
          { label: 'Neutral', data: [], backgroundColor: 'rgb(201, 203, 207)' },
          { label: 'Negative', data: [], backgroundColor: 'rgb(255, 99, 132)' },
        ],
    ...
    const displayLabels = sortedDates.map(formatDisplayLabel);
    
    // Create chart data
    const chartData: ChartData & { groupingMode?: string } = {
...
export const prepareUserDistributionData = (distribution: Record<string, number>): ChartData => {
  try {
    ...
    return {
      labels: [],
      datasets: [
        { label: 'Ratings', data: [], backgroundColor: 'rgb(54, 162, 235)' },
      ],
    };
  }
};